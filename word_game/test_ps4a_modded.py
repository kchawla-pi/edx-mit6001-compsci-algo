...
def test_playHand(testInput):
    for testNum, testData in testInput.items():
        for idx, info in enumerate(testData):
            pass
            # playHand(hand=info.origHand, wordList=wordList, n=info.handSize)  # Initiate test.
...
def createTestData():
    ...
    return {
        ...
        't7': (
            ...
            NewHandData(origHand={'a': 2, 'c': 1, 'e': 1, 'h': 1, 'm': 3, 'r': 1, 's': 1}, handSize=10),
            TestData(currentHand='a a e r h m m m c s', userInput='m', wordScore=None, handScore=None),
            TestData(currentHand='a a e r h m m m c s', userInput='chayote', wordScore=None, handScore=None),
            TestData(currentHand='a a e r h m m m c s', userInput='kwijibo', wordScore=None, handScore=None),
            TestData(currentHand='a a e r h m m m c s', userInput='hammer', wordScore=78, handScore=78),
